...
import { submitBookingReceipt, refreshBookingPaymentStatus } from './actions'
...
export default function BookingStatusClient({
  token, shortBookingId, customerFirstName, vendorName, vendorLogo,
  serviceName, checkIn, checkOut, nights, total,
  initialPaymentStatus, payments, waUrl, notifyPaidUrl,
}: Props) {
  ...
  // Poll for the vendor's decision only while waiting on them.
  useEffect(() => {
    if (status !== 'submitted') return
    const id = setInterval(async () => {
      const s = await refreshBookingPaymentStatus(token)
  ...
  const saveQr = (dataUrl: string) => {
    ...
    a.href     = dataUrl
    a.download = `pay-${shortBookingId}.png`
  ...
  const handleReceipt = async (file: File) => {
    ...
    try {
      ...
      const res = await submitBookingReceipt(token, fd)
      ...
      <div className="max-w-lg mx-auto px-4 py-8 space-y-5">
        ...
        <div className="flex justify-center pt-2">
          {vendorLogo ? (
            ...
            <div className="w-14 h-14 rounded-full bg-brand text-white font-bold text-lg flex items-center justify-center">
        ...
        {status === 'submitted' && (
          <div className="text-center space-y-1">
            ...
            <p className="text-sm text-fog">
              Your booking is being processed &mdash;{' '}
              <span className="font-semibold text-ink">{vendorName}</span> is confirming your payment.
            </p>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-border p-5 flex items-center justify-between">
          <div>
            <p className="text-xs text-fog">Booking reference</p>
            <p className="font-mono font-bold text-ink tracking-wider">{shortBookingId}</p>
          </div>
          <span className={`text-xs font-semibold px-3 py-1.5 rounded-full border ${BADGE[status].cls}`}>
            {BADGE[status].label}
          </span>
        </div>
        ...
        {status === 'confirmed' && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-6 text-center space-y-1">
            ...
            <p className="text-sm text-green-700">{vendorName} has confirmed your booking.</p>
          </div>
        )}

        {/* Booking summary */}
        <div className="bg-white rounded-2xl border border-border p-5 space-y-3">
          <h2 className="text-sm font-bold text-ink">Your booking</h2>
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-fog">Service</span>
              <span className="font-semibold text-ink">{serviceName}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-fog">Check-in</span>
              <span className="font-semibold text-ink tabular-nums">{fmtDate(checkIn)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-fog">Check-out</span>
              <span className="font-semibold text-ink tabular-nums">{fmtDate(checkOut)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-fog">Duration</span>
              <span className="text-ink">{nights} night{nights !== 1 ? 's' : ''}</span>
            </div>
          </div>
          {total > 0 && (
          ...
          <>
            {status === 'rejected' && (
              <div className="bg-red-50 border border-red-200 rounded-2xl p-4 text-sm text-brand">
                There was a problem with your payment. Please check you paid{' '}
                {total > 0 && <span className="font-bold">RM {amountStr}</span>} and upload your receipt again.
              </div>
            )}
              ...
              <div className="bg-white rounded-2xl border border-border p-5 space-y-4">
                  ...
                  {total > 0 && (
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-xs text-fog">
                        Pay <span className="font-bold text-ink">RM {amountStr}</span>
                      </p>
                      <button
                        onClick={copyAmount}
                ...
                <ol className="space-y-2.5 pt-1">
                  {[
                    ...
                    { n: '4', title: 'Upload your receipt',   desc: 'Screenshot your successful payment and upload it below to confirm your booking.' },
                  ].map(({ n, title, desc }) => (
            ...
            <div className="bg-white rounded-2xl border border-border p-5 space-y-3">
              ...
              {status === 'submitted' ? (
                <div className="space-y-3">
                  ...
                  {/* Single combined button: sends vendor a rich booking summary + receipt link */}
                  <a
                    href={notifyPaidUrl}
                    target="_blank"
                  ...
                  >
                    &#128242; Send booking summary to {vendorName}
                  </a>
                  <button
                ...
                <>
                  <p className="text-xs text-fog">
                    After paying, upload a screenshot of your receipt so {vendorName} can confirm.
                  </p>
                  <button
                    onClick={() => fileRef.current?.click()}
                    disabled={uploading}
...
function PaymentBlock({
  p, amountStr, showStatic, onToggleStatic, onSaveQr,
}: {
  p: PaymentView
  amountStr: string
  showStatic: boolean
    ...
    <div className="border border-border rounded-xl p-4 space-y-3">
      ...
      {m.type === 'bank' && (
        <div>
          ...
          <p className="text-sm text-fog">
            Account: <span className="font-mono font-semibold text-ink">{m.account_number}</span>
          </p>
      ...
      {p.dynamicQr && !showStatic && (
        <div className="flex flex-col items-center pt-1">
          <img
            src={p.dynamicQr}
            alt="Payment QR"
            className="w-48 h-48 object-contain rounded-xl border border-border bg-white"
          />
          <p className="text-xs text-green-700 font-semibold mt-2 text-center">
            RM {amountStr} is already filled in &mdash; just scan &amp; confirm
          </p>
          <div className="flex gap-4 mt-2">
            <button
              onClick={() => onSaveQr(p.dynamicQr!)}
      ...
      {/* Fallback: original static QR */}
      {((!p.dynamicQr && m.qr_url) || (showStatic && m.qr_url)) && (
        <div className="flex flex-col items-center pt-1">
          <img
            src={m.qr_url}
            alt="Standard QR"
            className="w-44 h-44 object-contain rounded-xl border border-border bg-white"
          />
          <p className="text-xs text-fog mt-2 text-center">
            Scan and enter <span className="font-bold text-ink">RM {amountStr}</span> yourself
          </p>
          {p.dynamicQr && (
            <button
              onClick={onToggleStatic}
              className="text-xs font-semibold text-fog underline underline-offset-2 mt-1"
            >
              &larr; Back to auto-fill QR
            </button>
          )}
        </div>
      )}
